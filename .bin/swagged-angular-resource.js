(function() {
  var apiUrl, async, fs, getJson, getResources, handlebars, log, outputFile, registerHelpers, request, url, _;

  fs = require("fs");

  request = require("request");

  url = require("url");

  _ = require("underscore");

  _.str = require("underscore.string");

  async = require("async");

  handlebars = require("handlebars");

  log = function() {
    return console.log.apply(this, arguments);
  };

  if (process.argv.length < 4) {
    return log("Expected: npm run build-service <endpoint> <file.js>");
  }

  registerHelpers = function(fns) {
    var candidateHelpers;
    candidateHelpers = _.filter(_.keys(fns), function(fnName) {
      return _.isFunction(fns[fnName]);
    });
    return _.each(candidateHelpers, function(fnName) {
      return handlebars.registerHelper(fnName, function() {
        var args;
        args = Array.prototype.slice.call(arguments, 0).slice(0, -1);
        return fns[fnName].apply(this, args);
      });
    });
  };

  registerHelpers(_.str);

  getJson = function(url, cb) {
    return request.get({
      url: url,
      json: true
    }, function(err, res, result) {
      return cb(result);
    });
  };

  getResources = function(apiUrl, outputFile) {
    return getJson(apiUrl.href, function(resourceApis) {
      resourceApis = _.filter(resourceApis.apis, function(apiResource) {
        return apiResource.path !== "/";
      });
      return async.reduce(resourceApis, [], function(memo, apiResource, cb) {
        var apiResourceUrl;
        apiResourceUrl = url.parse("" + apiUrl.href + apiResource.path);
        return getJson(apiResourceUrl.href, function(apiResourceObject) {
          _.each(apiResourceObject.apis, function(resourceApis) {
            return _.each(_.filter(resourceApis.operations, function(operation) {
              return operation.type !== null;
            }), function(operation) {
              var model, multipleNestedResource, singleNestedResource;
              model = apiResourceObject.models[operation.type];
              singleNestedResource = _.pick(model.properties, function(property) {
                return property.format && property.format.indexOf('/') > -1;
              });
              multipleNestedResource = _.pick(model.properties, function(property) {
                return property.items;
              });
              return memo.push({
                type: _.str.camelize(apiResource.path.replace('/', '')),
                nickname: _.str.classify(operation.nickname),
                isQuery: operation.method === "GET" && !operation.parameters,
                isGet: operation.method === 'GET' && operation.parameters !== void 0,
                isPost: operation.method === 'POST',
                isDelete: operation.method === 'DELETE',
                isPut: operation.method === 'PUT',
                isPatch: operation.method === 'PATCH',
                swaggerPath: resourceApis.path,
                angularPath: (resourceApis.path || '').replace(/\{(.+)\}/g, ":$1"),
                singleNestedResource: singleNestedResource,
                multipleNestedResource: multipleNestedResource
              });
            });
          });
          return cb(null, memo);
        });
      }, function(err, results) {
        var byApiResources, byNestedApiResource, code, data, tpl;
        byNestedApiResource = _.groupBy(_.filter(results, function(result) {
          return result.isGet;
        }), 'swaggerPath');
        _.each(results, function(result, k) {
          results[k].singleNestedResource = _.map(result.singleNestedResource, function(vs, ks) {
            var format, nested;
            format = vs.format.replace('{pk}', '{id}');
            nested = byNestedApiResource[format][0];
            return {
              name: ks,
              type: nested.type,
              nickname: nested.nickname
            };
          });
          return results[k].multipleNestedResource = _.map(result.multipleNestedResource, function(vm, ks) {
            var format, nested;
            format = vm.items.format.replace('{pk}', '{id}');
            nested = byNestedApiResource[format][0];
            return {
              name: ks,
              type: nested.type,
              nickname: nested.nickname
            };
          });
        });
        byApiResources = _.groupBy(results, 'type');
        data = {
          angularProviderType: "service",
          angularProviderSuffix: "ResourceService",
          apiUrl: "http:/" + apiUrl,
          apiTypes: _.keys(byApiResources),
          apiResources: byApiResources,
          apiNestedResources: byNestedApiResource
        };
        tpl = fs.readFileSync("src/templates/swagged-resources.hbs", {
          encoding: "utf-8"
        });
        code = handlebars.compile(tpl)(data);
        return fs.writeFileSync(process.argv[3], code, {
          flags: "w+"
        });
      });
    });
  };

  apiUrl = url.parse(process.argv[2]);

  outputFile = process.argv[3];

  getResources(apiUrl, outputFile);

}).call(this);
